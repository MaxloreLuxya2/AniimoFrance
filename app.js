(function () {
  ...
  function fetchLive() {
    fetch(TIER_API, { cache: "no-store" }).then(function (r) {
      ...
      LIVE.lists.forEach(function (l) {
        ...
        l._tok = tokOf(l.id) || (mine && mine._tok) || "";
        l._saved = true;
        byId[l.id] = l;
      });
  ...
  /* ---- propriété d'une Tiers List de joueur ----------------------------
     Le serveur attribue à chaque liste une clé de modification tirée au hasard
     et refuse tout enregistrement qui ne la présente pas : c'est elle, et non le
     pseudo, qui fait la preuve du propriétaire (deux joueurs peuvent porter le
     même pseudo, et un pseudo se devine).

     Ces clés — comme les listes elles-mêmes — sont rangées dans leurs propres
     entrées, minuscules. Le brouillon complet pèse plus de 10 Mo, largement
     au-dessus du quota du navigateur : son écriture échouait à chaque fois, et
     la clé disparaissait donc au rechargement. C'était la raison pour laquelle
     on ne pouvait pas revenir modifier sa liste. */
  var TOK_KEY = "aniimo.tiertok", MYL_KEY = "aniimo.mylists";
  function tokAll() {
    try { return JSON.parse(localStorage.getItem(TOK_KEY) || "{}") || {}; }
    catch (e) { return {}; }
  }
  function tokOf(id) { return tokAll()[id] || ""; }
  function tokSet(id, tok) {
    if (!id || !tok) return;
    var m = tokAll(); m[id] = tok;
    try { localStorage.setItem(TOK_KEY, JSON.stringify(m)); } catch (e) {}
  }
  function tokDel(id) {
    var m = tokAll(); delete m[id];
    try { localStorage.setItem(TOK_KEY, JSON.stringify(m)); } catch (e) {}
  }
  /* sauvegarde locale des seules listes du joueur : quelques kilo-octets, donc
     elle passe là où le brouillon complet est refusé. */
  function saveMine(msg) {
    try { localStorage.setItem(MYL_KEY, JSON.stringify(S.tierLists || [])); } catch (e) {}
    if (msg) toast(msg);
  }
  function loadMine() {
    try {
      var v = JSON.parse(localStorage.getItem(MYL_KEY) || "null");
      if (v && v.length && !(S.tierLists && S.tierLists.length)) S.tierLists = v;
    } catch (e) {}
  }
  ...
  /* propriétaire = je détiens la clé de cette liste, ou elle vient d'être créée
     ici et n'a pas encore été enregistrée (elle n'a donc pas encore de clé). */
  function tIsMine(id) {
    if (tokOf(id)) return true;
    return tLists().some(function (l) { return l.id === id && !l.shared && !l._saved; });
  }
  function tListOf(id) {
    var l = tAll();
  ...
  function viewTier() {
    ...
    /* les deux notes ci-dessous commentent le classement officiel : elles n'ont
       pas de sens sur une liste personnelle (« Créer » ou liste partagée), on
       les réserve donc à la Tiers List du site. */
    if (!list) {
    ...
    h += "</div>";
    h += votePanel();
    if (!list) h += goldNote("À noter", "Cette Tiers List vaut pour le Monde ouvert (Open World).");
  ...
  function tCard(r, ranked, role, list) {
    ...
    /* pas de draggable="true" : le glisser est piloté par les événements de
       pointeur (voir startPointerDrag), pour que la molette reste utilisable */
    return '<div class="tcard' + (picked ? " picked" : "") + '"' +
      (edit ? ' data-drag="' + esc(a.name) + '"' : "") +
  ...
  function bindAdminTiers() {
    ...
    document.querySelectorAll("[data-ldel]").forEach(function (b) {
      b.onclick = function () {
        ...
        apiPost(TIER_API, { action: "delete-list", id: l.id,
          editToken: tokOf(l.id) || l._tok || "", adminPass: adminPass() })
  ...
  /* Aucun glisser natif ne doit démarrer sur une vignette de la Tiers List.
     Les vignettes contiennent des <img>, et une image est glissable d'office :
     le navigateur lançait donc son propre glisser par-dessus le nôtre et, ce
     faisant, confisquait la molette (impossible de faire défiler la page en
     gardant le clic enfoncé). On le coupe systématiquement ici — le
     déplacement des vignettes est entièrement piloté par startPointerDrag. */
  document.addEventListener("dragstart", function (e) {
    if (e.target && e.target.closest && e.target.closest("[data-drag]")) { e.preventDefault(); return; }
  ...
  function renderRail() {
    ...
    h += "</div>" +
      '<div class="dcwrap"><p class="railupd">Le site sera mis à jour tous les dimanches.</p>' +
      discordCard() + "</div>" +
  ...
  function render() {
    ...
    saveView();
    if (t.kind === "tier") {
      /* on repositionne tout de suite, puis une seconde fois à la frame
         suivante : si la mise en page n'était pas encore stabilisée (images en
         cours de chargement), le premier appel se fait tronquer par le
         navigateur et la page saute. */
  ...
  /* Retrouve la vignette d'un Aniimo sans passer par un sélecteur CSS
     (les noms peuvent contenir des caractères qui casseraient la requête). */
  function cardEl(name) {
    var found = null;
    document.querySelectorAll("[data-drag]").forEach(function (c) {
      if (!found && c.dataset.drag === name) found = c;
    });
    return found;
  }

  /* Déplace la vignette DANS LE DOM au lieu de redessiner toute la page.
     C'est la clé du confort de création : un redessin complet oblige à
     recalculer puis restaurer la position de défilement, et c'est là que la
     page sautait. Ici rien n'est reconstruit, donc rien ne peut sauter.
     Renvoie false si le déplacement n'est pas possible : on retombe alors
     sur un rendu classique. */
  function moveCardDom(name, key) {
    var card = cardEl(name);
    if (!card) return false;
    var cell = card.closest(".tbcell");
    if (!cell || !cell.parentNode) return false;
    /* la colonne correspond au rôle : on doit rester dans la même */
    var col = Array.prototype.indexOf.call(cell.parentNode.children, cell);
    var target = null;
    document.querySelectorAll(".tbcell[data-drop]").forEach(function (c) {
      if (target || c.dataset.drop !== key || !c.parentNode) return;
      if (Array.prototype.indexOf.call(c.parentNode.children, c) === col) target = c;
    });
    if (!target || target === cell) return target === cell;
    target.appendChild(card);
    /* compteur « N placés » du titre, tenu à jour sans tout redessiner */
    var l = curList(), cnt = document.querySelector(".head .count");
    if (l && cnt) cnt.textContent = Object.keys(l.tiers || {}).length + " placés";
    return true;
  }

  /* Sélection : on se contente d'allumer la vignette, sans redessiner. */
  function pickCard(name) {
    view.tpick = view.tpick === name ? null : name;
    document.querySelectorAll(".tcard.picked").forEach(function (c) { c.classList.remove("picked"); });
    if (view.tpick) {
      var el = cardEl(view.tpick);
  ...
  function placeIn(name, key) {
    ...
    var wasPicked = view.tpick;
    view.tpick = null;
    saveMine();
    if (moveCardDom(name, key)) {
      if (wasPicked) {
        document.querySelectorAll(".tcard.picked").forEach(function (c) { c.classList.remove("picked"); });
      }
      saveView();
  ...
  function wireTier() {
    ...
    on("tsave", "onclick", function () {
      ...
      apiPost(TIER_API, {
        action: "save-list",
        list: { id: l.id, pseudo: l.pseudo, title: l.title, tiers: l.tiers,
                editToken: tokOf(l.id) || l._tok || "" }
      }).then(function (res) {
        if (btn) { btn.disabled = false; btn.textContent = "Enregistrer"; }
        if (res.d && res.d.ok) {
          /* la clé n'est renvoyée qu'au créateur, et une seule fois : on la range
             tout de suite, c'est elle qui rouvrira la liste en modification. */
          if (res.d.editToken) { l._tok = res.d.editToken; tokSet(l.id, res.d.editToken); }
          l._saved = true;
          saveMine();
          toast("« " + (l.title || "Ta liste") + " » est enregistrée sur le site et visible par tous. " +
            "Toi seul peux la modifier.");
          fetchLive();
        } else if (res.status === 403) {
          toast("Cette liste appartient à un autre navigateur : impossible de l'écraser depuis ici.");
        } else {
          saveMine("Liste enregistrée (hors-ligne)");
    ...
    on("tdel", "onclick", function () {
      ...
      S.tierLists = tLists().filter(function (x) { return x.id !== l.id; });
      S.tierPublic = tPublic().filter(function (x) { return x.id !== l.id; });
      view.tier = "ALL"; saveMine("Tiers list supprimée"); render();
      apiPost(TIER_API, { action: "delete-list", id: l.id,
        editToken: tokOf(l.id) || l._tok || "", adminPass: adminPass() })
        .then(function () { tokDel(l.id); })
        .then(function () { fetchLive(); }).catch(function () {});
    });
    on("tfork", "onclick", function () {
      ...
      saveMine("Tiers list reprise"); render();
    });
    ...
    document.querySelectorAll("[data-drag]").forEach(function (c) {
      c.addEventListener("click", function (e) {
        ...
        if (dragMoved) return;   /* fin d'un vrai glisser : ce n'est pas un clic de sélection */
        pickCard(c.dataset.drag);
      });
      c.addEventListener("pointerdown", startPointerDrag);
    });
  ...
  /* ---- glisser-déposer des vignettes de la Tiers List -------------------
     Volontairement écrit avec les événements de pointeur plutôt qu'avec le
     glisser-déposer natif HTML5 : pendant un glisser natif, le navigateur
     confisque la molette (aucun événement wheel n'est émis et la page ne
     défile pas), ce qui empêchait d'atteindre une bande hors de l'écran sans
     lâcher la vignette. Ici rien n'est confisqué : on garde le clic gauche
     enfoncé, on fait défiler à la molette, puis on relâche sur la bande visée.
     Le fantôme suit le curseur en coordonnées écran, il reste donc sous la
     souris pendant que la page défile. */
  var dragName = null, dragGhost = null, dragCell = null, dragMoved = false, dragFrom = null;
  var dragX = 0, dragY = 0;

  function startPointerDrag(e) {
    if (e.button !== 0 || e.pointerType === "touch") return;  /* tactile : on garde le tap */
    if (e.target.closest("[data-ani]")) return;               /* le bouton « i » reste cliquable */
    var card = e.currentTarget;
    dragFrom = { x: e.clientX, y: e.clientY, card: card, name: card.dataset.drag,
                 cell: card.closest("[data-drop]") };
  ...
  function onPointerDragMove(e) {
    if (!dragFrom) return;
    if (!dragMoved) {
      /* seuil de déclenchement : en deçà, c'est un clic de sélection et non un
         glisser. Assez large pour absorber le tremblement naturel de la main,
         qui sinon transformait chaque clic en glisser avorté. */
      if (Math.abs(e.clientX - dragFrom.x) + Math.abs(e.clientY - dragFrom.y) < 14) return;
      dragMoved = true;
      dragName = dragFrom.name;
      dragFrom.card.classList.add("dragging");
      dragGhost = dragFrom.card.cloneNode(true);
      dragGhost.className = "tcard tcghost";
      document.body.appendChild(dragGhost);
      document.body.classList.add("tdragging");
      /* dès qu'on tient une vignette, le pointeur est capturé : les événements
         continuent d'arriver même si le curseur sort de la vignette d'origine */
      try { dragFrom.card.setPointerCapture(e.pointerId); } catch (err) {}
    }
    e.preventDefault();
  ...
  /* la case survolée est recalculée à partir du point écran : elle reste donc
     juste même quand c'est la page, et non le curseur, qui a bougé. */
  function highlightCellAt(x, y) {
    ...
    if (dragCell) dragCell.classList.remove("over");
    dragCell = cell;
    if (dragCell) dragCell.classList.add("over");
  }

  /* Défilement à la molette PENDANT le glisser.
     On ne se repose pas sur le comportement par défaut du navigateur : selon
     l'état dans lequel il se croit (bouton maintenu, sélection en cours...), il
     lui arrive de l'ignorer. On prend donc la main — on annule son action et on
     fait défiler nous-mêmes — ce qui rend le geste fiable partout. */
  function onDragWheel(e) {
    if (!dragMoved) return;
    e.preventDefault();
    var d = e.deltaY;
    if (e.deltaMode === 1) d *= 16;                    /* défilement par lignes */
    else if (e.deltaMode === 2) d *= window.innerHeight; /* par pages */
    window.scrollBy(0, d);
    highlightCellAt(dragX, dragY);
  }
  ...
  function endPointerDrag(e) {
    ...
    var cell = dragCell, name = dragName, moved = dragMoved;
    var from = dragFrom ? dragFrom.cell : null;
    cleanupDrag();
    if (moved && name && e.type === "pointerup") {
      if (cell && cell !== from) placeIn(name, cell.dataset.drop);
      else {
        /* Relâché sans avoir changé de bande — c'est le cas quand la main
           tremble un peu entre l'appui et le relâchement. On ne jette pas le
           geste : l'Aniimo reste sélectionné, comme après un clic. On peut
           alors faire défiler tranquillement puis cliquer la bande voulue. */
        pickCard(name);
      }
    }
    /* le clic qui suit le relâchement ne doit pas rejouer la sélection */
    if (moved) setTimeout(function () { dragMoved = false; }, 0);
  }

  function cleanupDrag() {
    if (dragGhost && dragGhost.parentNode) dragGhost.parentNode.removeChild(dragGhost);
    if (dragCell) dragCell.classList.remove("over");
    if (dragFrom && dragFrom.card) dragFrom.card.classList.remove("dragging");
    document.body.classList.remove("tdragging");
    dragGhost = null; dragCell = null; dragName = null; dragFrom = null;
  }

  /* non passif : indispensable pour pouvoir annuler l'action par défaut */
  document.addEventListener("wheel", onDragWheel, { passive: false });
  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape" && dragMoved) { cleanupDrag(); dragMoved = false; }
  });
  /* filet de sécurité : si un glisser est interrompu autrement que par un
     relâchement propre (fenêtre qui perd le focus, glisser natif déclenché
     malgré tout), on remet l'état à zéro pour ne pas rester bloqué avec un
     fantôme collé au curseur et des clics qui ne répondent plus. */
  function abortDrag() {
    if (!dragFrom && !dragMoved) return;
    document.removeEventListener("pointermove", onPointerDragMove);
    document.removeEventListener("pointerup", endPointerDrag);
    document.removeEventListener("pointercancel", endPointerDrag);
    cleanupDrag(); dragMoved = false;
  }
  window.addEventListener("blur", abortDrag);
  /* Attention : un dragstart part AUSSI de nos propres vignettes, car elles
     contiennent une image et le navigateur tente d'en démarrer le glisser natif
     (on l'annule plus haut). Il ne faut donc surtout pas annuler notre glisser
     dans ce cas — c'était l'erreur : le geste mourait à l'instant même où il
     commençait, et la molette n'avait plus rien à faire défiler. On n'annule
     que pour un glisser natif venu d'ailleurs. */
  document.addEventListener("dragstart", function (e) {
    if (e.target && e.target.closest && e.target.closest("[data-drag]")) return;
    abortDrag();
  });
  ...
  function openShared(txt, go) {
    ...
    saveMine("Tiers list partagée ouverte");
  ...
  /* les Tiers List créées ici sont relues avant le premier affichage :
     elles réapparaissent donc telles quelles après un rafraîchissement. */
  loadMine();
  restoreView();
